...
def parse_tables(path: Path) -> dict[tuple, list[tuple[float, float, float]]]:
...
def plot_strict(model: dict, rules: dict, out_dir: Path) -> tuple[int, dict[str, int], int]:
    ...
    for key in shared:
        ...
        axs[0].plot(md, mcc, "s-", label="130.rcx.model")
        ...
        axs[1].plot(md, mcg, "s-", label="130.rcx.model")
        ...
        out_png = out_dir / filename_of_key(key)
...
def plot_multiwidth(
    model: dict, rules: dict, out_dir: Path, min_model_widths: int
    ...
    for hdr in shared_headers:
        ...
        for w in mwidths:
            ...
            axs[0].plot(md, mcc, "s-", alpha=0.8, label=f"130 w={w:g}")
            axs[1].plot(md, mcg, "s-", alpha=0.8, label=f"130 w={w:g}")
        ...
        out_png = out_dir / filename_of_header(hdr)
...
def main() -> None:
    ...
    model = parse_tables(model_path)
    rules = parse_tables(rules_path)
    if args.mode == "strict":
        shared_cnt, by_family, generated = plot_strict(model, rules, out_dir)
    else:
        shared_cnt, by_family, generated = plot_multiwidth(
            model, rules, out_dir, args.min_model_widths
        )
