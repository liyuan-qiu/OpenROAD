#!/usr/bin/env python3
"""
Summarize Nangate45 FasterCap model vs rules errors for Over / Under / OverUnder.
...
def agg_stats(vals: list[float]) -> dict:
    ...
    return {"n": len(vals), "mean": statistics.mean(vals), "median": statistics.median(vals), "p90": p90}
...
def build_report(points: list[dict], *, model_path: str, rules_path: str, dist_match_mode: str, allow_extrapolation: bool) -> str:
    lines = [
        "# Nangate45 model vs rules — Over / Under / OverUnder error summary",
        "",
        f"- Model: `{model_path}`",
        f"- Rules: `{rules_path}`",
    ...
    for fam in FAMILIES:
        ...
        lines += [
            f"## {fam}",
            f"- CC MAE mean={cc['mean']:.4g}, median={cc['median']:.4g}, p90={cc['p90']:.4g}; mean bias(model-rules)={cc_bias:+.4g}",
            f"- CG MAE mean={cg['mean']:.4g}, median={cg['median']:.4g}, p90={cg['p90']:.4g}; mean bias={cg_bias:+.4g}",
            "",
        ]
    ...
    if len(points) >= 3:
        ...
        lines += [
            "## Dist correlation",
            f"- Pearson(dist, CG signed error) = **{corr:.3f}** (positive ⇒ model CG rises with dist vs rules)",
            "",
        ]
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Analyze Nangate45 O/U/OU model vs rules errors")
    ap.add_argument("--model", required=True)
    ap.add_argument("--rules", required=True)
    ap.add_argument("--out-dir", required=True)
    ap.add_argument("--dist-match-mode", choices=["strict", "interp"], default="strict")
    ...
    for key in sorted(set(model).intersection(rules)):
        ...
        if not matched:
            continue
        points.extend({**meta, **r} for r in matched)
        key_rows.append(
            {
                **meta,
                "n_dist": len(matched),
    ...
    write_csv(out / "by_family.csv", list(fam_rows[0].keys()) if fam_rows else ["family"], fam_rows)
    ...
    write_csv(out / "worst_keys_cg.csv", list(worst[0].keys()) if worst else ["metal"], worst)
    ...
    plot_error_vs_dist(points, out / "cg_signed_error_vs_dist.png", yfield="cg_abs", title="CG signed error (model - rules) vs dist")
    plot_error_vs_dist(points, out / "cg_abs_error_vs_dist.png", yfield="cg_abs_m", title="CG |error| vs dist")
    plot_error_vs_dist(points, out / "cc_abs_error_vs_dist.png", yfield="cc_abs_m", title="CC |error| vs dist")
