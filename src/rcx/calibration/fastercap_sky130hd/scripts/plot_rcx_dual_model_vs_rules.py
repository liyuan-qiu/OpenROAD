...
def plot_strict_dual(
    ...
    for key in shared:
        ...
        fig.suptitle(f"{title_of_key(key)}\nCG_sub = TC - CC - CC2")
        ...
        axs[1].plot(ad, acg, "s--", alpha=0.9, label=f"{label_a} (FR=TC-CC)")
        axs[1].plot(bd, bcg, "^-", alpha=0.9, label=f"{label_b} (CG_sub)")
...
def main() -> None:
    ...
    ma = parse_tables(Path(args.model_a))
    mb = parse_tables(Path(args.model_b))
    rules = parse_tables(Path(args.rules))
