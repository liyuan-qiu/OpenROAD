#!/usr/bin/env bash
# Summarize Sky130 Over/Under/OverUnder model vs rules errors.
set -euo pipefail
...
python3 "${FC_DIR}/scripts/analyze_sky130_model_vs_rules_errors.py" \
  --model "${MODEL}" \
  --rules "${RULES}" \
  --out-dir "${OUT_DIR}" \
  "${EXTRA_ARGS[@]}"

echo "==> Sky130 O/U/OU error analysis -> ${OUT_DIR}"
