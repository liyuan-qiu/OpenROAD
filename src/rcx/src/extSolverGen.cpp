                                       ...
                                       double& max_x)
{
  ...
  double X0 = x - targetPitch;  // left near-neighbor spacing
  // xd[1] is innermost left wire (closest to center); xd[n] is farthest left.
  xd[1] = X0;
  double min_x = xd[1];
  for (int ii = 2; ii < n + 1; ii++) {
    xd[ii] = xd[ii - 1] - min_pitch;
